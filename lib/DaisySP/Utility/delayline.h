...
class DelayLine
{
  public:
    ...
    */
    // also updates the count_ for reverse delay;
    inline void Write(const T sample)
    {
        line_[write_ptr_] = sample;
        write_ptr_        = (write_ptr_ - 1 + max_size) % max_size;
        count_ = (count_ + 1) % delay_;
    }
    ...
    */
    ...
    /** returns the next sample of type T in the reversed delay line.
    */
    inline const T ReadReversed() const
    {
        T a = line_[(write_ptr_ + count_*2) % max_size];
        return a;
    } 
